...
export const action = async ({ request }) => {
  ...
  if (mode === "login") {
    ...
    const authString = userArr.find((user) => user.email === authData.email);
    //const userCurrent = authString.fullName.slice(0, 6);
    localStorage.setItem("userCurrent", JSON.stringify(userCurrent));
    // console.log(userCurrent);
    // const aaa = JSON.parse(localStorage.getItem("userCurrent")) ?? [];
